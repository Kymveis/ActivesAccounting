...
internal sealed class AddCurrencyViewModel : AddItemViewModelBase<ICurrency>
{
    ...
    private sealed record CurrencyTemplate
        (ICurrenciesContainer Container, string Name, CurrencyType Type) : ITemplate<ICurrency>
    {
        public bool HasDuplicate => Container.Currencies.Any(aC => Name.Equals(aC.Name) && Type.Equals(aC.Type));
        public ICurrency ToItem() => Container.CreateCurrency(Name, Type);
    }

    public AddCurrencyViewModel(
        ICurrenciesContainer aCurrenciesContainer,
        Action aCloseAction,
        Action<ICurrency> aNewItemAction)
    ...
    public CurrencyType SelectedType { get; set; }
    public ObservableCollection<CurrencyType> CurrencyTypes { get; }

    protected override ITemplate<ICurrency> ValidateFields(ICollection<string> aErrors)
    {
        if (string.IsNullOrWhiteSpace(Name))
        {
            aErrors.Add("Name is not set.");
        }

        return aErrors.Any()
            ? CreateInvalidTemplate()
            : new CurrencyTemplate(_currenciesContainer, Name, SelectedType);
    }
}